...
@section('content')
    ...
    <div class="alert alert-warning">
        This page's content is currently outdated and needs to be updated to reflect the incorporation of FusionInvoice
        by the InvoicePlane project. Please visit the
        <a href="https://community.invoiceplane.com/">Community Forum</a> for further help.
    </div>

    <ul>
        <li><a href="#how-to-upgrade-invoicePlane-2018">How to upgrade InvoicePlane 2018</a></li>
        <li><a href="#how-to-upgrade-to-invoicePlane-2018-from-invoicePlane-2017-2016">How to upgrade to
                InvoicePlane 2018 from InvoicePlane 2017 or 2016</a></li>
        <li><a href="#how-to-upgrade-to-invoicePlane-2018-from-invoicePlane-v2">How to upgrade to
                InvoicePlane 2018 from InvoicePlane v2</a></li>
        <li><a href="#how-to-upgrade-an-add-on">How to upgrade an Add-on</a></li>
    </ul>

    <hr>

    <span class="anchor" id="how-to-upgrade-invoicePlane-2018"></span>
    <h3>How to Upgrade InvoicePlane 2018</h3>

    <p>This section is for upgrading from InvoicePlane 2018-x to InvoicePlane 2018-x.
        If you are upgrading from 2017 or 2016, see <a
                href="#how-to-upgrade-to-invoicePlane-2018-from-invoicePlane-2017-2016">here</a>.
        If you are upgrading from v2, see <a href="#how-to-upgrade-to-invoicePlane-2018-from-invoicePlane-v2">here</a>.
    </p>

    <h4>Step 1: Download the latest package</h4>
    <p>Log into your account at InvoicePlane.com and download the latest package. Save it locally to your
        computer.</p>
    ...
    <p>Upload the following folders to the InvoicePlane folder on your server and let them merge/overwrite the
        existing folders:</p>
    ...
    <p>It is recommended that you use a standard FTP program such as
        <a href="https://filezilla-project.org/download.php?type=client" target="_blank">FileZilla</a> to upload
        the files
        to your server.
    </p>
    ...
    <p>
        Visit http://YourInvoicePlaneURL/setup in your web browser. If your http://YourInvoicePlaneURL/setup
        produces an error, try using
        http://YourInvoicePlaneURL/index.php/setup instead. This step will upgrade your database. Once this
        step is completed, you
        should be able to log back into your upgraded InvoicePlane system.
    </p>

    <hr>

    <span class="anchor" id="how-to-upgrade-to-invoicePlane-2018-from-invoicePlane-2017-2016"></span>
    <h3>How to Upgrade to InvoicePlane 2018 from InvoicePlane 2017 or 2016</h3>
    ...
    <p>Delete these folders and files from the InvoicePlane installation on your server:</p>
    ...
    <p>
        ...
        value with
        your license key listed in your InvoicePlane.com account panel and save the modified file. Be sure
        there are no
        leading or trailing spaces.
    </p>

    <p>
        Open config/database.php from the unzipped installer package, edit accordingly for your database
        settings and save
        the modified file.
    </p>

    <h4>Step 5: Upload the folders and files</h4>
    <p>Upload the following folders and files from the installer package to the InvoicePlane folder on your
        server:</p>
    ...
    <p>
        Visit <code>http://YourInvoicePlaneURL/setup</code> in your web browser. If your <code>http://YourInvoicePlaneURL/setup</code>
        produces an error, try using <code>http://YourInvoicePlaneURL/index.php/setup</code> instead. This step will upgrade your database. Once this step is completed, you should be able to log back into your upgraded InvoicePlane system.
    </p>

    <h4>Step 7: If upgrading from InvoicePlane 2016</h4>

    <p>If you are upgrading from InvoicePlane 2017, then you are all done! Happy Invoicing!</p>

    <p>If you are upgrading from InvoicePlane 2016 and are using custom quote and/or invoice templates, there
        are some changes you'll need to make. If any of these exist in your custom template(s), they should be
    ...
    <span class="anchor" id="how-to-upgrade-to-invoicePlane-2018-from-invoicePlane-v2"></span>
    <h3>How to Upgrade to InvoicePlane 2018 from InvoicePlane v2</h3>
    ...
    <h4>Step 2: Delete all existing InvoicePlane files and folders from your server</h4>

    <p>Delete the entire folder from your server which contains the InvoicePlane folders and files. Recreate
        the empty folder once it has been deleted.</p>
    ...
    <p>
        Open config/database.php from the unzipped installer package. Edit the file and point it to your
        existing
        InvoicePlane database and save the modified file. Use your backed up database.php file for reference
        purposes.
    </p>

    <p>
        Typically you should only have to configure the host, database, username and password values to connect
        to your
        database. Compatibility with MySQL and MariaDB is 100% guaranteed. Other database types may or may not
        work as expected and are
        not supported.
    </p>
...
'key' => 'ReplaceThisWithYourOwnLicenseKey',
    ...
    <p>
        Upload the unzipped files from your computer to the empty folder on your server you recreated earlier.
        It is
    ...
    <p>The exact steps to set the appropriate permissions will depend on your web host and server configuration.
        InvoicePlane cannot advise on the exact steps or permissions to apply to make these folders writable.
        If you have
    ...
    <p>
        Visit http://YourInvoicePlaneURL/setup in your web browser. If http://YourInvoicePlaneURL/setup
        produces an error, try using
        http://YourInvoicePlaneURL/index.php/setup instead. This step will upgrade your v2 database. Once this
        step is completed, you
        should be able to log into your upgraded InvoicePlane system.
    </p>

    <h4>Step 10: System Settings</h4>

    <p>
        Once logged into InvoicePlane, go to System -> System Settings and update the following settings:
    </p>
    ...
    <ol>
        ...
        <li>If you had online payments configured in v2, review the Online Payments tab and adjust accordingly.
            Use your
    ...
    <p>
        ...
        InvoicePlane v2 and would like to use those same templates in InvoicePlane 2018. Below are the steps
        to follow,
    ...
    <p>Save your changes and upload the file to custom/templates/invoice_templates if it's an invoice template
        or to
    ...
    <p>Log into your account at InvoicePlane.com and download the add-on package to upgrade. Save it locally to
        your computer.</p>
    ...
    <p>
        Upload the unzipped add-on folder from your computer to the custom/addons folder on your server and let
        it merge/overwrite the
    ...
    <p>Log into your InvoicePlane install and go to System -> Add-ons and press the Upgrade button for the
        add-on if it appears. If
        the Upgrade button doesn't appear, then no further action is required and the add-on should be upgraded
        and ready to use.</p>
